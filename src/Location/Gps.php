...
final class Gps
{
	...
	public static function fromString(string $value): array
	{
		...
		$found = Strings::match($value, '~^(\d{1,3}\.\d+?)(N|S), ?(\d{1,3}\.\d+?)(E|W)$~i');
		if ($found !== null) {
			/** @var array<non-empty-string> $found */
		...
		$found = Strings::match($value, '~(-?\d{1,3}\.\d+), ?(-?\d{1,3}\.\d+)$~');
		if ($found !== null) {
			/** @var array<non-empty-string> $found */
		...
		$found = Strings::match($value, '~^(N|S) ?(\d{1,3})°(\d{1,2}\.\d+?)\',? ?(W|E) ?(\d{1,3})°(\d{1,2}\.\d+?)\'$~i');
		if ($found !== null) {
			/** @var array<non-empty-string> $found */
		...
		$found = Strings::match($value, '~^(\d{1,3})°(\d{1,2})\'(\d{1,2}\.\d+?)"(N|S), ?(\d{1,3})°(\d{1,2})\'(\d{1,2}\.\d+?)"(W|E)$~i');
		if ($found !== null) {
			/** @var array<non-empty-string> $found */
		...
		// N49.20811° E19.04247°
		$found = Strings::match($value, '~^(N|S)(\d{1,3}\.\d+?)° ?(E|W)(\d{1,3}\.\d+?)°$~i');
		if ($found !== null) {
			/** @var array<non-empty-string> $found */
			return self::coordinate(self::checkCoordinate((float) $found[2], $found[1]), self::checkCoordinate((float) $found[4], $found[3]));
		}

		throw InvalidArgumentsException::createUnsupportedCoordinate($value);
	}
	 ...
	 *
	 * @param non-empty-string $pole
	 *
	 * @throws InvalidArgumentsException
	 */
	private static function checkCoordinate(
		float $num,
		string $pole,
	): float
	{
		switch (strtoupper($pole)) {
			...
			case 'N':
				...
				break;
			default:
				throw InvalidArgumentsException::createUnsupportedPole($pole);
		}

		if ($num > 180) {
			throw InvalidArgumentsException::createCoordinateOutOfRange($num);
		}

		return $num;
	}

	private static function degToDec(
		int|float $degrees,
		int|float $minutes,
		int|float $seconds = 0.0,
	): float
	{
